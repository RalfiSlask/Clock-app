...
const popup_container = document.querySelector(".more-container");
const popup_button = document.querySelector(".icon");
const popup_button_text = document.querySelector(".moreless");
const time_container = document.querySelector(".time-container");
const city_and_country = document.querySelector(".location");
...
const clickingPopupButton = () => {
    popup_button.onclick = () => {
        quote_container.classList.toggle("hidden");
        popup_container.classList.toggle("hidden");
        if(isButtonClicked == false) {
            popup_button.classList.add("less");
            popup_button.classList.remove("more");
            popup_button_text.innerHTML = "LESS";
            popup_button.innerHTML = `<svg width="40" height="40" xmlns="http://www.w3.org/2000/svg" class="arrow"><g fill="none" fill-rule="evenodd"><circle fill="#303030" cx="20" cy="20" r="20"/><path stroke="#FFF" stroke-width="2" d="M14 23l6-6 6 6"/></g></svg>`
            isButtonClicked = true;
           
        } else if(isButtonClicked == true){
            popup_button.classList.remove("less");
            popup_button.classList.add("more");
...
const getTimeAPI = async () => {
    try {
        ...
        const changeGreetingLogo = () => {
            // sun icon 5am - 6pm / moon icon 6pm - 5am
            if(timenumber < 5) {
        ...
        let greet = "";
        //  Good morningbetween 5am - 12pm / Good afternoon 12pm - 6pm / Good Evening 6pm - 5am
        ...
        const changeBackgroundImage = () => {
            if(greet == "GOOD EVENING") {
                backgroundImage.style.backgroundImage = "url(./assets/desktop/bg-image-nighttime.jpg)"
            } else {
                backgroundImage.style.backgroundImage = "url(./assets/desktop/bg-image-daytime.jpg)"
            }
        }
...
const getCountryCity = async () => {
    try {
        ...
        city_and_country.innerHTML = `IN ${city}, ${country}`
        console.log(city)
    } catch (error) {
        console.log(error);
    }
